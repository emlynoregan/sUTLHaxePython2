...
class Sutl:
	...
	def builtins(self):
		...
		def _hx_local_13(parentscope13,scope13,l13,src13,tt13,b15,h13):
			...
			if condvalue:
				...
				retval = _g._evaluate(parentscope13,Util.get(scope13,"false"),l13,src13,tt13,b15,h13)
			return retval
	...
	def logenter(self,msg,s,t,h):
		if (h > 0):
			haxe_Log.trace(((("(" + Std.string(h)) + "): ") + ("null" if msg is None else msg)),_hx_AnonObject({'fileName': "Sutl.hx", 'lineNumber': 549, 'className': "Sutl", 'methodName': "logenter"}))
			haxe_Log.trace((" - s: " + HxOverrides.stringOrNull(haxe_format_JsonPrinter.xprint(s,None,"  "))),_hx_AnonObject({'fileName': "Sutl.hx", 'lineNumber': 550, 'className': "Sutl", 'methodName': "logenter"}))
			haxe_Log.trace((" - t: " + HxOverrides.stringOrNull(haxe_format_JsonPrinter.xprint(t,None,"  "))),_hx_AnonObject({'fileName': "Sutl.hx", 'lineNumber': 551, 'className': "Sutl", 'methodName': "logenter"}))

	def logexit(self,msg,r,h):
		if (h > 0):
			haxe_Log.trace(((("(" + Std.string(h)) + "): ") + ("null" if msg is None else msg)),_hx_AnonObject({'fileName': "Sutl.hx", 'lineNumber': 559, 'className': "Sutl", 'methodName': "logexit"}))
			haxe_Log.trace((" - r: " + HxOverrides.stringOrNull(haxe_format_JsonPrinter.xprint(r,None,"  "))),_hx_AnonObject({'fileName': "Sutl.hx", 'lineNumber': 560, 'className': "Sutl", 'methodName': "logexit"}))
	...
	def _compilelib(self,decls,dists,l,b):
		...
		while (_g < len(decls)):
			...
			if (hasattr(decl,(("_hx_" + "requires") if ("requires" in python_Boot.keywords) else (("_hx_" + "requires") if (((((len("requires") > 2) and ((ord("requires"[0]) == 95))) and ((ord("requires"[1]) == 95))) and ((ord("requires"[(len("requires") - 1)]) != 95)))) else "requires"))) and Util2.isArray(Util.get(decl,"requires"))):
				reqnames = None
				def _hx_local_0():
...
class Tests_isType(haxe_unit_TestCase):
	...
	def isType(self,aTypeName,aObj):
		...
		self.assertTrue((Util2.isArray(aObj) == ((aTypeName == "array"))),_hx_AnonObject({'fileName': "Tests_isType.hx", 'lineNumber': 21, 'className': "Tests_isType", 'methodName': "isType"}))
		self.assertTrue((Util2.isString(aObj) == ((aTypeName == "string"))),_hx_AnonObject({'fileName': "Tests_isType.hx", 'lineNumber': 22, 'className': "Tests_isType", 'methodName': "isType"}))
		self.assertTrue((Util2.isNumber(aObj) == ((aTypeName == "number"))),_hx_AnonObject({'fileName': "Tests_isType.hx", 'lineNumber': 23, 'className': "Tests_isType", 'methodName': "isType"}))
		self.assertTrue((Util2.isBool(aObj) == ((aTypeName == "bool"))),_hx_AnonObject({'fileName': "Tests_isType.hx", 'lineNumber': 24, 'className': "Tests_isType", 'methodName': "isType"}))
...
class haxe_unit_TestResult:
	...
	def toString(self):
		...
		_g_val = [None]
		while (_g_head[0] is not None):
			test = None
			def _hx_local_0():
				_g_val[0] = (_g_head[0][0] if 0 < len(_g_head[0]) else None)
				_g_head[0] = (_g_head[0][1] if 1 < len(_g_head[0]) else None)
				return _g_val[0]
			test = _hx_local_0()
...
class haxe_unit_TestRunner:
	...
	def run(self):
		...
		while (_g_head[0] is not None):
			...
			c = _hx_local_0()
			self.runCase(c)
		haxe_unit_TestRunner.xprint(self.result.toString())
	...
	def runCase(self,t):
		...
		haxe_unit_TestRunner.xprint((("Class: " + HxOverrides.stringOrNull(Type.getClassName(cl))) + " "))
		_g = 0
		while (_g < len(fields)):
	...
	def customTrace(v,p = None):
		haxe_unit_TestRunner.xprint((((((HxOverrides.stringOrNull(p.fileName) + ":") + Std.string(p.lineNumber)) + ": ") + Std.string(v)) + "\n"))
